...
options:
    ...
    src:
        description:
          ...
          - Supports basic globbing for folders in the form of M(/*), which allows extraction of all the files & subfolders of the specified M(src) path.
          - When the globbing is used, then the M(force) option is automatically assumed for the contents of that given folder. I.e. the contents is extracted every time.
    ...
    paths:
        description:
          ...
          - Each element of the list is a YAML dictionary, which accepts these
            options: M(src) | M(dest) | M(owner) | M(group)
          - See the same options above for their description.
          - If this option is used, then at least M(src) must be specified for each member of the list.
        required: false
        default: null
        type: list
...
def dir_members(tarfile, src, dest, globbed=False):
    ...
    # Compensate for '/' divider for any subfolders / files inside the folder:
    subpath_len += 1

    for member in tarfile.getmembers():
        if member.name.startswith(src) and (member.isdir() or member.isfile()):
            if dest is None:
...
def extract_path(src, dest=None, owner=None, group=None, force=False):
...
def run_module():
    ...
    # Check that we full paths when 'chdir' was not set:
    if chdir is None:
    ...
    for path in paths:
        ...
        try:
            extracted_list = extract_path(src, dest, owner, group, force)
